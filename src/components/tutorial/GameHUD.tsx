...
export const GameHUD: React.FC = () => {
  ...
  return (
    <div className="fixed top-3 left-1/2 -translate-x-1/2 z-[9995] max-w-[95vw] w-auto animate-fade-in-up">
      <div className="bg-[#262220]/95 backdrop-blur-md text-white border border-[#d97706]/40 rounded-full px-4 py-2 shadow-2xl flex items-center gap-3 sm:gap-4 ring-2 ring-[#d97706]/20">
        {/* Left: Journey Badge */}
        <div className="flex items-center gap-2">
          <span className="w-6 h-6 rounded-full bg-[#d97706] text-white flex items-center justify-center text-xs font-black shadow-sm">
            🌱
          </span>
          <div className="hidden sm:block">
            <span className="text-[10px] font-black uppercase tracking-wider text-[#fbbf24] block leading-none">
              {language === 'hi' ? 'शिल्प यात्रा' : language === 'te' ? 'శిల్ప యాత్ర' : 'Artisan Journey'}
            </span>
            <span className="text-xs font-bold text-[#faf7f2] block truncate max-w-[140px]">
              {missionTitle}
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-1 min-w-[100px] sm:min-w-[160px]">
          <div className="flex items-center justify-between text-[11px] font-bold text-[#d6d3d1]">
            <span>
              {language === 'hi' ? 'लेवल' : language === 'te' ? 'లెవల్' : 'Level'} {currentLevel}/{totalLevels}
            </span>
            <span className="text-[#fbbf24] font-mono text-[10px]">{progressPct}%</span>
          </div>

          <div className="w-full h-1.5 bg-[#3d3835] rounded-full overflow-hidden">
            <div
              className="h-full bg-gradient-to-r from-[#d97706] to-[#ea580c] rounded-full transition-all duration-500 ease-out"
